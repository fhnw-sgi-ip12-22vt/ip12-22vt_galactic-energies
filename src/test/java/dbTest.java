...
public class dbTest {
    ...
    public void testEnergydata() throws SQLException {
        ...
        while (rs.next()) {
            assertEquals("You could use a Smartphone for", rs.getString("devicename"));
            assertEquals(20, rs.getInt("power"));
        }
        while (rs2.next()) {
            assertEquals("You could use a Dishwasher for", rs2.getString("devicename"));
            assertEquals(1250, rs2.getInt("power"));
        }
        while (rs3.next()) {
            assertEquals("You could use WhatsApp for", rs3.getString("devicename"));
